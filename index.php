...
try {
    $pdo = new PDO("mysql:host=localhost;dbname=lamplesson;charset=utf8","root","password"
...
if ($item_name && isset($_POST["btn"])) {
    ...
    $stmt = $pdo -> prepare("INSERT INTO index_data(name, point) VALUES (:name, :point)");
    $stmt->bindParam(':name', $item_name, PDO::PARAM_STR);
    $stmt->bindValue(':point', $item_point, PDO::PARAM_INT);
    $stmt->execute();
    }
    $select_sql = "SELECT * FROM index_data";
